document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchVideoDetails(videoId) {
        ...
        const isShortByUrl = videoUrls.some(u => u.includes('/shorts/') && getVideoId(u) === videoId);
        const category = (durationSec <= 60 || isShortByUrl) ? 'short' : 'long';
    ...
    async function fetchChannelDetails(channelId) {
        ...
        const channel = {
            ...
            subCount: parseInt(stats.subscriberCount || '0')
        };
    ...
    async function loadAllVideos() {
        ...
        if (videoUrls.length === 0) {
            loadingMessage.textContent = 'No videos added yet – paste some YouTube links in the videoUrls array.';
            return;
        }
